...
public class CharacterDaoImpl implements CharacterDao {
    ...
    @Override
    public List<Character> findAllByGameId(Integer id) {

        String sql = "SELECT * FROM `characters` JOIN `games_characters` ON characters.id = games_characters.character_id WHERE game_id = ?";
        List<Object> params = List.of(id);
